...
# Pandas'ın verebileceği gereksiz veritabanı uyarılarını gizleriz
warnings.filterwarnings('ignore')

# --- 1. SAYFA AYARLARI VE GÜVENLİK (ŞİFRELİ GİRİŞ) ---
st.set_page_config(page_title="Hukuk Otomasyonu", page_icon="⚖️", layout="wide")

if 'giris_yapildi' not in st.session_state:
    st.session_state.giris_yapildi = False

# 🌟 GİRİŞ EKRANI (ŞİFRE: Avukat2026)
if not st.session_state.giris_yapildi:
    st.title("🔒 Hukuk Otomasyonu - Güvenli Giriş")
    st.markdown("---")
    sifre = st.text_input("Lütfen Sistem Şifrenizi Girin:", type="password")
    if st.button("Giriş Yap", type="primary"):
        if sifre == "Avukat2026":  # Şifreni buradan değiştirebilirsin!
            st.session_state.giris_yapildi = True
            st.rerun()
        else:
            st.error("Hatalı şifre girdiniz!")
    st.stop() # Şifre girilmeden aşağıya geçmesini engeller

# --- 2. BULUT VERİTABANI (POSTGRESQL) BAĞLANTISI ---
DB_URL = "postgresql://neondb_owner:npg_vM0gdb9HIPSa@ep-empty-truth-asctpsnz-pooler.c-4.eu-central-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
def veritabani_kur():
    conn = get_conn()
    c = conn.cursor()
    # PostgreSQL tablolarında id için AUTOINCREMENT yerine SERIAL kullanılır
    c.execute('''CREATE TABLE IF NOT EXISTS davalar (id SERIAL PRIMARY KEY, esas_no TEXT, mahkeme TEXT, tur TEXT, tarih TEXT, taraf TEXT, durum TEXT, avukat TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS personeller (id SERIAL PRIMARY KEY, ad_soyad TEXT, gorev TEXT, telefon TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS notlar (id SERIAL PRIMARY KEY, icerik TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS durusmalar (id SERIAL PRIMARY KEY, esas_no TEXT, tarih TEXT, saat TEXT, aciklama TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS evraklar (id SERIAL PRIMARY KEY, esas_no TEXT, evrak_adi TEXT, yuklenme_tarihi TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS vekaletler (id SERIAL PRIMARY KEY, v_no TEXT, muvekkil TEXT, n_adi TEXT, y_no TEXT, n_tarih TEXT, durum TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS raporlar (id SERIAL PRIMARY KEY, isim TEXT, tarih TEXT, durum TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS finans (id SERIAL PRIMARY KEY, islem_turu TEXT, miktar REAL, aciklama TEXT, tarih TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS gorevler (id SERIAL PRIMARY KEY, gorev_adi TEXT, sorumlu TEXT, son_tarih TEXT, durum TEXT)''')
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO personeller (ad_soyad, gorev, telefon) VALUES ('Ayşe (Asistan)', 'İdari İşler', '0555 333 4455')")
...
if 'dava_duzenle_modu' not in st.session_state: st.session_state.dava_duzenle_modu = False
...
col_logo, col_cikis = st.columns([9, 1])
with col_logo:
...
with tab_dava:
    ...
    if st.session_state.aktif_dava_karti is not None:
        ...
        if df_dava[df_dava["Dosya Esas No"] == secilen].empty:
        ...
        with col_geri:
            if st.button("⬅️ Geri Dön", key="dava_kart_geri"):
                st.session_state.aktif_dava_karti = None; st.session_state.dava_duzenle_modu = False; st.rerun()
        with col_baslik: st.subheader(f"📂 Dava Kartı Detayı: {secilen}")
        with col_duzenle:
            if not st.session_state.dava_duzenle_modu:
                if st.button("✏️ Düzenle", use_container_width=True, key="btn_duzenle_mod"):
                    st.session_state.dava_duzenle_modu = True; st.rerun()
        st.markdown("---")
        ...
        if st.session_state.dava_duzenle_modu:
            ...
            with cd_g:
                if st.button("❌ İptal", key="e_iptal"): st.session_state.dava_duzenle_modu = False; st.rerun()
            with cd_i:
                if st.button("💾 Değişiklikleri Kaydet", type="primary", key="e_kaydet"):
                    conn = get_conn(); c = conn.cursor()
                    c.execute('''UPDATE davalar SET mahkeme=%s, tur=%s, tarih=%s, taraf=%s, durum=%s, avukat=%s WHERE esas_no=%s''', 
                              (yeni_mah, yeni_tur, yeni_tar.strftime("%d.%m.%Y"), yeni_taraf, yeni_durum, yeni_avu, secilen))
                    conn.commit(); conn.close(); st.session_state.dava_duzenle_modu = False; st.rerun()
        else:
            ...
            with tab_durusma:
                ...
                with st.expander("➕ Yeni Duruşma Ekle"):
                    ...
                    if st.button("Duruşmayı Kaydet", type="primary", key="btn_dur_kaydet"):
                        conn = get_conn(); c = conn.cursor()
                        c.execute("INSERT INTO durusmalar (esas_no, tarih, saat, aciklama) VALUES (%s, %s, %s, %s)", (secilen, d_tarih.strftime("%d.%m.%Y"), d_saat.strftime("%H:%M"), d_aciklama))
                ...
                with col_e2:
                    ...
                    if st.button("Evrak Kaydet", type="primary", key="btn_evrak_kaydet"):
                        if yuklenen_dosya is not None:
                            conn = get_conn(); c = conn.cursor()
                            c.execute("INSERT INTO evraklar (esas_no, evrak_adi, yuklenme_tarihi) VALUES (%s, %s, %s)", (secilen, yuklenen_dosya.name, datetime.date.today().strftime("%d.%m.%Y")))
        ...
        if not st.session_state.dava_sihirbaz:
                    ...
                    with st.expander(f"📌 {secilen_dava} Esas Numaralı Dosyanın Detayları", expanded=True):
                        ...
                        with col_islem:
                            ...
                            if st.button("🗑️ Dava Kartını Sil", use_container_width=True, type="primary", key=f"sil_{secilen_dava}"):
                                conn = get_conn(); c = conn.cursor()
                                c.execute("DELETE FROM davalar WHERE esas_no=%s", (secilen_dava,))
                                c.execute("DELETE FROM durusmalar WHERE esas_no=%s", (secilen_dava,))
                                c.execute("DELETE FROM evraklar WHERE esas_no=%s", (secilen_dava,))
                                conn.commit(); conn.close(); st.rerun()
        else:
            ...
            if st.session_state.dava_adim == 1:
                ...
                with c2:
                    ...
                    conn = get_conn(); df_pers = pd.read_sql_query("SELECT ad_soyad FROM personeller", conn); conn.close()
                ...
                if c_i.button("✅ Dava Kartını Kaydet", type="primary", key="d_kaydet_son"):
                    ...
                    conn = get_conn(); c = conn.cursor()
                    c.execute("INSERT INTO davalar (esas_no, mahkeme, tur, tarih, taraf, durum, avukat) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
# ==========================================
# 3. BÜRO & GÖREV YÖNETİMİ
# ==========================================
with tab_buro:
    st.subheader("🏢 Büro Yönetimi & Personel")
    conn = get_conn()
    df_personel = pd.read_sql_query("SELECT ad_soyad as \"Ad Soyad\", gorev as \"Görevi\", telefon as \"Telefon\" FROM personeller", conn)
    df_gorev = pd.read_sql_query("SELECT id, gorev_adi, sorumlu, son_tarih FROM gorevler WHERE durum='Bekliyor'", conn)
    df_notlar = pd.read_sql_query("SELECT id, icerik FROM notlar", conn)
    conn.close()
    
    c1, c2 = st.columns([6, 4])
    with c1:
        st.markdown("#### 👥 Avukatlar & Personel")
        st.dataframe(df_personel, use_container_width=True, hide_index=True)
        col_btn_ekle, col_btn_sil = st.columns(2)
        with col_btn_ekle:
            with st.expander("➕ Personel Ekle"):
                ...
                if st.button("Kaydet", type="primary", key="p_kaydet"):
                    if p_ad:
                        conn = get_conn(); c = conn.cursor()
                        c.execute("INSERT INTO personeller (ad_soyad, gorev, telefon) VALUES (%s, %s, %s)", (p_ad, p_gorev, p_tel))
                        conn.commit(); conn.close(); st.rerun()
        with col_btn_sil:
            with st.expander("🗑️ Personel Sil"):
                if not df_personel.empty:
                    ...
                    if st.button("Sil", key="p_btn_sil"):
                        conn = get_conn(); c = conn.cursor()
                        c.execute("DELETE FROM personeller WHERE ad_soyad=%s", (silinecek,)); conn.commit(); conn.close(); st.rerun()
        ...
        if df_gorev.empty: st.success("Harika! Bekleyen görev yok.")
            ...
            for index, row in df_gorev.iterrows():
                ...
                cg1.write(f"📌 **{row['gorev_adi']}** *(Sorumlu: {row['sorumlu']})*")
                cg2.write(f"⏳ {row['son_tarih']}")
                if cg3.button("✔", key=f"tamamla_{row['id']}"):
                    conn = get_conn(); c = conn.cursor()
                    c.execute("UPDATE gorevler SET durum='Tamamlandı' WHERE id=%s", (row['id'],)); conn.commit(); conn.close(); st.rerun()
        with st.expander("➕ Yeni Görev Ata"):
            ...
            if st.button("Görevi Ata", type="primary", key="g_ata"):
                if g_adi:
                    conn = get_conn(); c = conn.cursor()
                    c.execute("INSERT INTO gorevler (gorev_adi, sorumlu, son_tarih, durum) VALUES (%s, %s, %s, %s)", (g_adi, g_sorumlu, g_tarih.strftime("%d.%m.%Y"), "Bekliyor"))
                    conn.commit(); conn.close(); st.rerun()

    with c2:
        st.markdown("#### 📌 İlan Panosu & Notlar")
        if df_notlar.empty: st.info("Panoda hiç not yok.")
        else:
            for index, row in df_notlar.iterrows():
                cn1, cn2 = st.columns([9, 1])
                cn1.warning(row['icerik'])
                if cn2.button("🗑️", key=f"sil_not_{row['id']}"):
                    conn = get_conn(); c = conn.cursor(); c.execute("DELETE FROM notlar WHERE id=%s", (row['id'],)); conn.commit(); conn.close(); st.rerun()
        st.markdown("---")
        yeni_not = st.text_area("Hızlı Not Ekle:", placeholder="Ofis içi hızlı bir not bırakın...")
        if st.button("📝 Notu Panoya As", type="primary", key="btn_not_ekle"):
            if yeni_not:
                conn = get_conn(); c = conn.cursor(); c.execute("INSERT INTO notlar (icerik) VALUES (%s)", (f"📌 {yeni_not}",)); conn.commit(); conn.close(); st.rerun()
    ...
    with cf1:
        ...
        with st.form("finans_form", clear_on_submit=True):
            ...
            if f_kaydet and f_miktar > 0 and f_aciklama:
                conn = get_conn(); c = conn.cursor()
                c.execute("INSERT INTO finans (islem_turu, miktar, aciklama, tarih) VALUES (%s, %s, %s, %s)", (f_tur, f_miktar, f_aciklama, f_tarih.strftime("%d.%m.%Y")))
                conn.commit(); conn.close(); st.rerun()
    with cf2:
        ...
        if df_finans.empty: st.info("Kayıt bulunmuyor.")
            ...
            with st.expander("🗑️ İşlem Sil"):
                ...
                if st.button("Sil", type="primary", key="btn_f_sil"):
                    conn = get_conn(); c = conn.cursor()
                    c.execute("DELETE FROM finans WHERE aciklama=%s", (sil_islem,))
                    conn.commit(); conn.close(); st.rerun()
    ...
    if not st.session_state.vek_sihirbaz:
        ...
        if st.session_state.vek_adim == 1:
            ...
            if st.button("✅ Kaydet", type="primary", key="v_kaydet_son"):
                conn = get_conn(); c = conn.cursor()
                c.execute("INSERT INTO vekaletler (v_no, muvekkil, n_adi, y_no, n_tarih, durum) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    if not st.session_state.rap_sihirbaz:
        ...
        if st.session_state.rap_adim == 1:
            ...
            if st.button("✅ Kaydet", type="primary", key="r_kaydet_son"):
                conn = get_conn(); c = conn.cursor()
                c.execute("INSERT INTO raporlar (isim, tarih, durum) VALUES (%s, %s, %s)", (st.session_state.gecici_rapor.get('isim'), datetime.date.today().strftime("%d.%m.%Y"), "Hazır"))
            ...
            with c_girdi:
                ...
                if st.button("🖩 Hesapla", type="primary", key="btn_hesap_2"):
                    if asil is not None:
                        ...
                        else: st.error("Bitiş tarihi hatalı!")
